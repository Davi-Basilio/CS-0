...
const coronhaRifle = new THREE.Mesh(
...
const bolaVerde = new THREE.Mesh(
  new THREE.SphereGeometry(0.045, 16, 16),
  new THREE.MeshStandardMaterial({ color: 0x00ff00, roughness: 0.5 }),
);
const topoCilindro = new THREE.Mesh(
  new THREE.CylinderGeometry(0.012, 0.012, 0.03),
  new THREE.MeshStandardMaterial({ color: 0x888888 }),
);
topoCilindro.position.y = 0.055;
const argolaGranada = new THREE.Mesh(
  new THREE.TorusGeometry(0.016, 0.004, 8, 16),
...
// 7. Mini Uzi
const grupoMiniUzi = new THREE.Group();
const materialMetalUzi = new THREE.MeshStandardMaterial({
  color: 0x1a1a1a,
  roughness: 0.5,
});
const materialCanoUzi = new THREE.MeshStandardMaterial({
  color: 0x0d0d0d,
  roughness: 0.4,
});

// Corpo/receiver — bem retangular e compacto, característica da família Uzi/MAC
const corpoUzi = new THREE.Mesh(
  new THREE.BoxGeometry(0.045, 0.08, 0.16),
  materialMetalUzi,
);
corpoUzi.position.set(0, 0.02, -0.03);

// Cano/guarda-mão vazado saindo pra frente
const canoUzi = new THREE.Mesh(
  new THREE.CylinderGeometry(0.013, 0.013, 0.18),
  materialCanoUzi,
);
canoUzi.rotation.x = Math.PI / 2;
canoUzi.position.set(0, 0.015, -0.2);

// Carregador comprido e vertical — a marca registrada visual desse tipo de arma,
// serve quase como a empunhadura da arma
const carregadorUzi = new THREE.Mesh(
  new THREE.BoxGeometry(0.022, 0.15, 0.028),
  materialMetalUzi,
);
carregadorUzi.position.set(0, -0.09, -0.01);
carregadorUzi.rotation.x = -0.05;

// Empunhadura, logo atrás do carregador
const empunhaduraUzi = new THREE.Mesh(
  new THREE.BoxGeometry(0.028, 0.09, 0.04),
  materialMetalUzi,
);
empunhaduraUzi.position.set(0, -0.02, 0.03);
empunhaduraUzi.rotation.x = 0.35;

// Mira traseira, bloquinho no topo
const miraTraseiraUzi = new THREE.Mesh(
  new THREE.BoxGeometry(0.015, 0.012, 0.015),
  materialCanoUzi,
);
miraTraseiraUzi.position.set(0, 0.075, 0.02);
...
grupoArmasPlayer.add(grupoMiniUzi);
